      ...
      response => {
        if (response.likes) {
          ...
          if (page === 1) {
            ...
            this.publications.forEach((like: any) => {
              this.countLikes(like.publication._id).then((value) => {
                like.publication.stats = value;
              }).catch((err) => {
                console.log(err);
              });
            });
    ...
    this.likeService.addLike(this.token, params).subscribe(
      response => {
        if (response.like) {
          ...
          this.publications.forEach((like: any) => {
            if (like.publication._id === publication._id) {
              like.publication.stats += 1;
            }
          });
          this.status = 'success';
        } else {
